...
export const GET = (request: NextRequest) => {
  const callbackUrl = new URL(getAuthApiUrl('auth/facebook/callback'))
  const requestHostname = (request.headers.get('host') || request.nextUrl.host).split(':')[0] || request.nextUrl.hostname
