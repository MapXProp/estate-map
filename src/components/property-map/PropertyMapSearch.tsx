...
export default function PropertyMapSearch({
  ...
  const [projectSearchKey, setProjectSearchKey] = useState(requestKey)
  if (projectSearchKey !== requestKey) {
    setProjectSearchKey(requestKey)
  ...
  const closeMapProject = () => {
    ...
    if (id)
      window.requestAnimationFrame(() => {
        document
          .querySelector<HTMLAnchorElement>(`[data-mapx-project-id="${CSS.escape(id)}"] a`)
