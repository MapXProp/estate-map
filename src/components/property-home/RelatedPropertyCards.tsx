...
export default function RelatedPropertyCards({
  listings,
  heading,
  ...
  recommendationDetails?: Record<number, { reason: string; offers: string[] }>
}) {
    ...
    <>
      {heading !== false && (
        <h2 className="mb-5 font-sarabun text-xl font-semibold">
          {heading || (th ? 'อสังหาฯ ที่คุณอาจสนใจ' : 'More properties to explore')}
        </h2>
      )}
            ...
            <li key={item.id} className="relative min-w-0">
              ...
              >
                ...
                <div className="p-2.5 min-[500px]:p-4">
                  {recommendationDetails?.[item.id] && (
                    <p
                      data-recommendation-reason
                      className="mb-2 line-clamp-2 text-xs leading-5 text-[#176b50] dark:text-emerald-300"
                    >
                      {recommendationDetails[item.id].reason}
                    </p>
                  )}
                  <PropertyPrices
                    prices={filterPropertyPrices(getPropertyPrices(item), recommendationDetails?.[item.id]?.offers)}
              ...
              {recommendationDetails?.[item.id] && (
                <BtnLikeIcon
                  listingIdentifier={item.slug}
                  className="absolute top-2 right-2 shadow-sm"
                  sizeClass="size-11"
                  colorClass="bg-white/95 text-neutral-700 hover:bg-white"
                />
              )}
            </li>
          )
