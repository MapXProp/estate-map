...
export default function AuthModal({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!normalizedEmail.includes('@')) {
      setError(isThai ? 'กรุณากรอกอีเมลให้ถูกต้อง' : 'Enter a valid email address.')
      return
    }
    if (mode === 'signup' && !isStrongPassword(password)) {
      setError(isThai ? 'รหัสผ่านยังไม่ครบตามเงื่อนไขด้านล่าง' : 'Your password does not meet all requirements below.')
      return
    }
    if (mode === 'login' && !password) {
      setError(isThai ? 'กรุณากรอกรหัสผ่าน' : 'Enter your password.')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (mode === 'signup' && response.status === 409) {
          throw new Error(
            isThai ? 'อีเมลนี้มีบัญชีอยู่แล้ว กรุณาเข้าสู่ระบบ' : 'This email already has an account. Please sign in.'
          )
        }
        throw new Error(
          data?.error ||
            (mode === 'signup'
              ? isThai
                ? 'ยังสมัครสมาชิกไม่ได้ กรุณาลองอีกครั้ง'
  ...
  const title = isListing
    ? isSignup
      ...
      : isThai
        ...
        : 'Sign in to continue your listing'
    : isSignup
      ? isThai
        ? 'สร้างบัญชี MapxProp'
        : 'Create your MapxProp account'
      : isThai
  ...
  const submitLabel = isListing
    ...
    : isSignup
      ...
      : isThai
        ? 'เข้าสู่ระบบ'
        : 'Sign in'
  const submittingLabel = isSignup
    ? isThai
      ? 'กำลังลงทะเบียน…'
      : 'Creating account…'
    : isThai
  ...
  return (
    <Dialog open={open} onClose={handleClose} className="relative z-[100]">
      <DialogBackdrop
        transition
        className="fixed inset-0 bg-neutral-950/35 backdrop-blur-[2px] transition duration-200 data-closed:opacity-0"
      />
      <div className="fixed inset-0 overflow-y-auto p-3 sm:p-6">
        <div className="flex min-h-full items-center justify-center min-[744px]:-translate-y-6">
          <DialogPanel
            transition
            className="relative w-full max-w-md rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_28px_90px_rgba(15,23,42,0.24)] transition duration-200 sm:p-7 dark:border-neutral-700 dark:bg-neutral-900 data-closed:translate-y-3 data-closed:scale-[0.98] data-closed:opacity-0"
          >
            <button
              type="button"
              onClick={handleClose}
              ...
              className="absolute end-4 top-4 grid size-9 place-items-center rounded-full bg-neutral-100 text-neutral-500 transition hover:bg-neutral-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-neutral-800 dark:text-neutral-300"
            >
            ...
            <DialogTitle className="mt-5 px-8 text-center font-sarabun text-xl font-semibold text-neutral-900 dark:text-neutral-50">
              {title}
            </DialogTitle>
            ...
            <div className="mt-7 grid gap-2">
              <a
                ...
                onClick={(event) => handleSocialAuth(event, 'google')}
                aria-busy={socialSubmitting === 'google'}
                aria-disabled={Boolean(socialSubmitting)}
                className={`inline-flex h-11 items-center justify-center gap-2 rounded-2xl border px-3 font-sarabun text-sm font-medium transition duration-150 active:scale-[0.985] ${socialSubmitting === 'google' ? 'border-[#9bcabb] bg-[#e8f4ef] text-[#176b50]' : 'border-neutral-200 bg-white text-neutral-700 hover:border-[#b8d9cd] hover:bg-[#f1f8f5] hover:text-[#176b50] dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-200 dark:hover:border-emerald-800 dark:hover:bg-emerald-950/40 dark:hover:text-emerald-200'} ${socialSubmitting && socialSubmitting !== 'google' ? 'pointer-events-none opacity-50' : ''}`}
              >
              ...
              <a
                href={socialAuthUrl('line')}
                onClick={(event) => handleSocialAuth(event, 'line')}
                aria-busy={socialSubmitting === 'line'}
                aria-disabled={Boolean(socialSubmitting)}
                className={`inline-flex h-11 items-center justify-center gap-2 rounded-2xl border px-3 font-sarabun text-sm font-medium transition duration-150 active:scale-[0.985] ${socialSubmitting === 'line' ? 'border-[#9bcabb] bg-[#e8f4ef] text-[#176b50]' : 'border-neutral-200 bg-white text-neutral-700 hover:border-[#b8d9cd] hover:bg-[#f1f8f5] hover:text-[#176b50] dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-200 dark:hover:border-emerald-800 dark:hover:bg-emerald-950/40 dark:hover:text-emerald-200'} ${socialSubmitting && socialSubmitting !== 'line' ? 'pointer-events-none opacity-50' : ''}`}
              >
                {socialSubmitting === 'line' ? <AuthLoadingSpinner /> : <LineIcon className="size-5 shrink-0" />}
                {socialSubmitting === 'line'
                  ? isThai
                    ? 'กำลังเชื่อมต่อ LINE…'
                    : 'Connecting to LINE…'
                  : isSignup
            ...
            <div className="relative my-3 text-center">
              <span className="relative z-10 bg-white px-3 font-sarabun text-xs text-neutral-400 dark:bg-neutral-900">
                {isThai ? 'หรือใช้อีเมล' : 'or use email'}
              </span>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-3">
                <label className="block">
                  <span className="font-sarabun text-sm font-medium text-neutral-700 dark:text-neutral-200">
                    {isThai ? 'อีเมล' : 'Email'}
                  </span>
                  <input
                    type="email"
                    ...
                    className="mt-1.5 h-12 w-full rounded-2xl border border-neutral-200 bg-white px-4 text-base text-neutral-900 transition outline-none placeholder:text-neutral-400 focus:border-[#147458] focus:ring-4 focus:ring-[#dceee7] sm:text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100 dark:focus:border-emerald-500 dark:focus:ring-emerald-950/70"
                  />
                </label>
                ...
                <label className="block">
                  ...
                  <span className="relative mt-1.5 block">
                    ...
                    <button
                      ...
                      className="absolute inset-y-0 end-1.5 grid w-10 place-items-center rounded-xl text-neutral-400 transition hover:text-neutral-600 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-neutral-400 dark:text-neutral-500 dark:hover:text-neutral-300"
                    >
                  ...
                  {isSignup ? (
                    <span
                      ...
                      aria-label={isThai ? 'เงื่อนไขรหัสผ่าน' : 'Password requirements'}
                    >
                      {passwordRules.map((rule) => (
                        <span
                          key={rule.label}
                          className={`inline-flex items-center gap-1 rounded-md px-2 py-1 font-mono text-[11px] font-medium transition-colors ${rule.passed ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:ring-emerald-800' : 'bg-neutral-100 text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400'}`}
                        >
                          {rule.passed ? <CheckIcon className="size-3 stroke-[2.5]" /> : null}
                          {rule.label}
                        </span>
                      ))}
                    </span>
                ...
                {mode === 'login' ? (
                  <a
                    ...
                    className="block text-right font-sarabun text-xs text-neutral-500 hover:underline"
                  >
                ...
                {error ? (
                  <p
                    role="alert"
                    className="rounded-xl bg-red-50 px-3 py-2 font-sarabun text-sm text-red-600 dark:bg-red-950/30 dark:text-red-300"
                  >
                    {error}
                  </p>
                ) : null}
              ...
              <button
                ...
                className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#176b50] px-5 font-sarabun text-sm font-semibold text-white shadow-[0_8px_20px_rgba(23,107,80,0.18)] transition-colors hover:bg-[#125b44] focus-visible:ring-2 focus-visible:ring-[#5c9c87] focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-wait disabled:opacity-75"
              >
            ...
            <p className="mt-4 text-center font-sarabun text-sm text-neutral-500 dark:text-neutral-400">
              ...
              <button
                type="button"
                onClick={() => selectMode(isSignup ? 'login' : 'signup')}
                disabled={submitting || Boolean(socialSubmitting)}
                className="font-semibold text-neutral-800 underline-offset-4 hover:underline disabled:opacity-50 dark:text-neutral-100"
              >
                {isSignup ? (isThai ? 'เข้าสู่ระบบ' : 'Sign in') : isThai ? 'สมัครสมาชิก' : 'Create account'}
              </button>
            </p>
          </DialogPanel>
        </div>
