      ...
      <div className="space-y-7 p-5 sm:p-7">
        <DetailGroup title={isThai ? 'ห้องและวันพร้อมเข้าอยู่' : 'Room & availability'}>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Field label={isThai ? 'รูปแบบห้อง' : 'Room type'}>
              <Select name="roomTypeCode" defaultValue={readText(draft.roomTypeCode)} className="[&_select]:h-11 [&_select]:rounded-2xl">
            ...
            <Field label={isThai ? 'จำนวนห้องว่าง' : 'Rooms available'}>
              <Input name="availableRoomCount" type="number" min="0" defaultValue={readText(draft.availableRoomCount)} placeholder="1" />
            </Field>
            ...
            <Field label={isThai ? 'ลักษณะห้องน้ำ' : 'Bathroom arrangement'}>
              <Select name="bathroomType" defaultValue={readText(draft.bathroomType)} className="[&_select]:h-11 [&_select]:rounded-2xl">
            ...
            <Field label={isThai ? 'สภาพห้อง' : 'Room condition'}>
              <Select name="propertyCondition" defaultValue={readText(draft.propertyCondition)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
        ...
        <DetailGroup title={isThai ? 'ค่าใช้จ่ายก่อนเข้าอยู่และค่าสาธารณูปโภค' : 'Move-in costs & utilities'}>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label={isThai ? 'ค่าเช่าล่วงหน้า (เดือน)' : 'Advance rent (months)'}>
              <Input name="advanceRentMonths" type="number" min="0" step="1" defaultValue={readText(draft.advanceRentMonths)} placeholder="1" />
            </Field>
            <MoneyField name="utilityDepositAmount" label={isThai ? 'ประกันค่าน้ำ/ไฟ' : 'Utility deposit'} draft={draft} />
            <Field label={isThai ? 'การคิดค่าน้ำ' : 'Water billing'}>
              <Select name="waterBillingType" defaultValue={readText(draft.waterBillingType)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
            ...
            <MoneyField name="waterRate" label={isThai ? 'ค่าน้ำต่อหน่วย/เดือน' : 'Water rate per unit/month'} draft={draft} />
            <Field label={isThai ? 'การคิดค่าไฟ' : 'Electricity billing'}>
              <Select name="electricityBillingType" defaultValue={readText(draft.electricityBillingType)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
                <option value="included">{isThai ? 'รวมในค่าเช่า' : 'Included'}</option>
            ...
            <MoneyField name="electricityRate" label={isThai ? 'ค่าไฟต่อหน่วย' : 'Electricity rate per unit'} draft={draft} />
            ...
            <MoneyField name="parkingFeeMonthly" label={isThai ? 'ค่าที่จอดรถต่อเดือน' : 'Monthly parking fee'} draft={draft} />
        ...
        <DetailGroup title={isThai ? 'นโยบายผู้พัก' : 'Resident policies'}>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Field label={isThai ? 'สัตว์เลี้ยง' : 'Pets'}>
              <Select name="Pets" defaultValue={readText(draft.Pets)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
            ...
            <Field label={isThai ? 'การสูบบุหรี่' : 'Smoking'}>
              <Select name="smokingPolicy" defaultValue={readText(draft.smokingPolicy)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
            ...
            <Field label={isThai ? 'ผู้เช่าต่างชาติ' : 'Foreign tenants'}>
              <YesNoSelect name="foreignTenantAllowed" defaultValue={readText(draft.foreignTenantAllowed)} isThai={isThai} />
            </Field>
          </div>
        ...
        {propertyTypeCode === 'rental_room' ? (
          <DetailGroup title={isThai ? 'ข้อมูลห้องแบ่งเช่า' : 'Room-in-property details'}>
            <CheckboxGrid name="sharedFacilities[]" options={sharedFacilities} selected={readValues(draft['sharedFacilities[]'])} isThai={isThai} />
        ...
        {propertyTypeCode === 'apartment' ? (
          <DetailGroup title={isThai ? 'ข้อมูลอพาร์ตเมนต์' : 'Apartment details'}>
            {isServicedApartment ? (
              <>
                <CheckboxGrid name="servicesIncluded[]" options={services} selected={readValues(draft['servicesIncluded[]'])} isThai={isThai} />
                <div className="grid gap-5 sm:grid-cols-2">
                  <Field label={isThai ? 'ความถี่แม่บ้าน/เปลี่ยนผ้า' : 'Housekeeping / linen frequency'}>
                    <Input name="housekeepingFrequency" defaultValue={readText(draft.housekeepingFrequency)} placeholder={isThai ? 'เช่น สัปดาห์ละ 2 ครั้ง' : 'e.g. twice a week'} />
                  </Field>
                  <Field label={isThai ? 'เวลาให้บริการ Reception' : 'Reception hours'}>
                    <Input name="receptionHours" defaultValue={readText(draft.receptionHours)} placeholder="08:00–20:00" />
                  </Field>
                </div>
              </>
            ) : null}
        ...
        {propertyTypeCode === 'dormitory' ? (
          <DetailGroup title={isThai ? 'ข้อมูลหอพัก' : 'Dormitory details'}>
            <CheckboxGrid name="residentGroups[]" options={residentGroups} selected={readValues(draft['residentGroups[]'])} isThai={isThai} />
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label={isThai ? 'เวลาเข้าออก' : 'Curfew / access hours'}>
                <Input name="curfewTime" defaultValue={readText(draft.curfewTime)} placeholder={isThai ? 'เช่น เข้าได้ตลอด 24 ชม.' : 'e.g. 24-hour access'} />
              </Field>
          ...
          <DetailGroup title={isThai ? 'ข้อมูลแฟลตและสิทธิการเข้าอยู่' : 'Flat & occupancy rights'}>
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label={isThai ? 'หน่วยงานหรือผู้ดูแลโครงการ' : 'Managing agency'}>
                <Input name="managingAgency" defaultValue={readText(draft.managingAgency)} placeholder={isThai ? 'เช่น การเคหะแห่งชาติ' : 'e.g. National Housing Authority'} />
              </Field>
              <Field label={isThai ? 'ลักษณะสิทธิ' : 'Occupancy right'}>
                <Select name="occupancyRightType" defaultValue={readText(draft.occupancyRightType)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                  <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
              ...
              <Field label={isThai ? 'โอนสิทธิได้หรือไม่' : 'Rights transferable'}>
                <YesNoSelect name="rightsTransferAllowed" defaultValue={readText(draft.rightsTransferAllowed)} isThai={isThai} />
              </Field>
            </div>
          ...
          <DetailGroup title={isThai ? 'ข้อมูลคอนโดให้เช่ารายเดือน' : 'Monthly condo details'}>
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label={isThai ? 'รวมค่าส่วนกลางในค่าเช่าหรือไม่' : 'Common fee included'}>
                <YesNoSelect name="commonFeeIncluded" defaultValue={readText(draft.commonFeeIncluded)} isThai={isThai} />
              </Field>
              <Field label={isThai ? 'สถานะห้องปัจจุบัน' : 'Current occupancy'}>
                <Select name="occupancyStatus" defaultValue={readText(draft.occupancyStatus)} className="[&_select]:h-11 [&_select]:rounded-2xl">
                  <option value="">{isThai ? 'ไม่ระบุ' : 'Not specified'}</option>
          ...
          <DetailGroup title={isThai ? 'ข้อมูลโรงแรมรายเดือน' : 'Monthly hotel details'}>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label={isThai ? 'รอบทำความสะอาด/เปลี่ยนผ้า' : 'Cleaning / linen schedule'}>
                <Input name="housekeepingFrequency" defaultValue={readText(draft.housekeepingFrequency)} placeholder={isThai ? 'เช่น ทุก 3 วัน' : 'e.g. every 3 days'} />
              </Field>
              <Field label={isThai ? 'เวลาให้บริการ Reception' : 'Reception hours'}>
                <Input name="receptionHours" defaultValue={readText(draft.receptionHours)} placeholder="24 hours" />
              </Field>
            </div>
...
const MoneyField = ({ name, label, draft }: { name: string; label: string; draft: ListingDraft }) => (
  <Field label={label}>
    <div className="relative">
      <Input name={name} defaultValue={readText(draft[name])} inputMode="decimal" pattern="[0-9,]*(\.[0-9]{1,2})?" placeholder="0" className="pe-16!" />
      <span className="pointer-events-none absolute inset-y-0 end-4 flex items-center font-sarabun text-xs text-neutral-500">THB</span>
    </div>
  </Field>
)
...
const CheckboxGrid = ({
  ...
  <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
    {options.map((option) => (
      <label key={option.code} className="flex cursor-pointer items-center gap-3 rounded-2xl border border-sky-100 bg-white p-3 dark:border-sky-900/60 dark:bg-neutral-900">
        <input type="checkbox" name={name} value={option.code} defaultChecked={selected.includes(option.code)} className="peer sr-only" />
        ...
        <span className="font-sarabun text-sm text-neutral-700 dark:text-neutral-200">{isThai ? option.th : option.en}</span>
      </label>
    ))}
  </div>
)
