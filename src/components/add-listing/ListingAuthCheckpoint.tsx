...
const ListingAuthCheckpoint = ({ open, onClose, onAuthenticated }: Props) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'signup' && !isStrongPassword(password)) {
      setError('รหัสผ่านต้องมีอย่างน้อย 8 ตัว พร้อมตัวพิมพ์ใหญ่ พิมพ์เล็ก ตัวเลข และสัญลักษณ์')
      return
    }
  ...
  const socialAuthUrl = (provider: 'google' | 'line') => {
    ...
    url.searchParams.set('redirect', '/add-listing/2')
    return url.toString()
  }

  return (
    <Dialog open={open} onClose={submitting ? () => undefined : onClose} className="relative z-50">
      <DialogBackdrop
        transition
        ...
        <div className="flex min-h-full items-center justify-center">
          ...
          >
            ...
            <DialogTitle className="mt-5 px-8 text-center font-sarabun text-xl font-semibold text-neutral-900 dark:text-neutral-50">
              {isSignup ? 'สมัครเพื่อลงประกาศต่อ' : 'เข้าสู่ระบบเพื่อลงประกาศต่อ'}
            </DialogTitle>

            <div className="mt-7 grid gap-2">
              <a
                href={socialAuthUrl('google')}
                className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-neutral-200 bg-white px-3 font-sarabun text-sm font-medium text-neutral-700 transition hover:border-neutral-300 hover:bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-200 dark:hover:bg-neutral-800"
              >
            ...
            <form onSubmit={handleSubmit}>
                ...
                <label className="block">
                  ...
                  <span className="relative mt-1.5 block">
                    <input
                      ...
                      placeholder={isSignup ? 'อย่างน้อย 8 ตัวอักษร' : 'กรอกรหัสผ่าน'}
                  ...
                  {isSignup ? (
                    <span
                      ...
                      aria-label="อย่างน้อย 8 ตัว มีตัวพิมพ์ใหญ่ ตัวพิมพ์เล็ก ตัวเลข และสัญลักษณ์"
                    >
                      {passwordRules.map((rule) => (
                        <span
                          key={rule.label}
                          className={`inline-flex items-center gap-1 rounded-md px-2 py-1 font-mono text-[11px] font-medium transition-colors ${
                            rule.passed
              ...
              >
                {submitting ? 'กำลังดำเนินการ...' : isSignup ? 'สมัครและลงประกาศต่อ' : 'เข้าสู่ระบบและทำต่อ'}
...
export default ListingAuthCheckpoint
