...
const initialCategories = (url) => plain(map.initialMapCategories(
  { discoveryChannels: [url.searchParams.get('channel')] }, url.searchParams.getAll('category')
))

test('mobile cards use the current map taxonomy with one card representing the complete retail section', () => {
  assert.deepEqual(['homes', 'rooms', 'business'].map(channel => api.mobilePropertyCategories.filter(c => c.channel === channel).length), [7, 6, 9])
  for (const group of map.mapCategoryGroups) {
    const cards = api.mobilePropertyCategories.filter(card => card.channel === group.code)
    assert.deepEqual(plain(cards.flatMap(card => card.categoryIds)).sort(), plain(group.options.map(option => option.id)).sort())
...
test('every selected card opens exactly its map categories, including linked shared types', () => {
  for (const card of api.mobilePropertyCategories) {
    ...
    assert.deepEqual(initialCategories(url).sort(), plain(map.normalizeMapCategories(card.categoryIds)).sort(), card.value)
...
test('retail expands to all selling-space types and combines with a building category without adding keyword filters', () => {
  const retail = map.mapCategoryGroups.find(group => group.code === 'business').sections.find(section => section.id === 'retail')
  const query = '  สาทร & A/B?offer_type=sale #โครงการ  '
  const url = toUrl({ query, channel: 'business', selectedCategories: ['business:retail', 'business:office'], offerType: 'rent', minPrice: 20000, maxPrice: 60000 })
  ...
  assert.deepEqual(initialCategories(url).sort(), [...plain(retail.options.map(option => option.id)), 'business:office'].sort())
})

test('empty card selection scopes to the chosen channel and rooms remain rental-only', () => {
  for (const group of map.mapCategoryGroups) {
    const url = toUrl({ query: '', channel: group.code, selectedCategories: [], offerType: 'sale', minPrice: -1, maxPrice: Infinity })
    assert.deepEqual(initialCategories(url).sort(), plain(map.normalizeMapCategories(group.options.map(option => option.id))).sort())
...
function harness(locale = 'th') {
  const slots = []
  let cursor = 0, tree, propertyZone = 'homes'
  const component = load('src/components/property-home/MobilePropertySearch.tsx', {
    ...
    'rc-slider': { default: 'test-slider' },
    '@headlessui/react': { Dialog: 'test-dialog', DialogPanel: 'test-panel', DialogTitle: 'test-title' },
    '@/components/preferences/PreferencesProvider': { usePreferences: () => ({ locale, propertyZone, setPropertyZone: value => { propertyZone = value } }) },
  ...
  function render() { cursor = 0; tree = component({}) }
  ...
  function one(predicate) { const found = nodes(predicate); assert.equal(found.length, 1); return found[0] }
  function click(node) { node.props.onClick(); render() }
  render()
  return {
    nodes, one, click, render,
    data: (name, value) => one(node => node.props[name] === value),
    url: (query = '') => new URL(one(node => node.type === 'test-omnibox').props.buildSearchUrl(query), 'https://mapxprop.com'),
  }
}
...
test('the dialog passes the chosen budget as numeric map filters and leaves an open upper bound unlimited', () => {
  const h = harness('en')
  h.click(h.data('data-mobile-property-search-trigger', true))
  h.click(h.data('data-mobile-search-category', 'homes:semi_detached_house'))
  h.click(h.data('data-mobile-search-budget', true))
  h.click(h.data('data-mobile-budget-offer', 'rent'))
  h.click(h.data('data-mobile-budget-preset', '10000-20000'))
  h.click(h.one(node => node.type === 'button' && node.props.children === 'Use this budget'))
  let url = h.url()
  assert.deepEqual(url.searchParams.getAll('offer_type'), ['rent'])
  assert.deepEqual(url.searchParams.getAll('category'), ['homes:semi_detached_house'])
  assert.equal(url.searchParams.get('price_min'), '10000')
  assert.equal(url.searchParams.get('price_max'), '20000')
  h.click(h.data('data-mobile-search-budget', true))
  h.click(h.data('data-mobile-budget-offer', 'sale'))
  h.click(h.data('data-mobile-budget-preset', '10000000-100000000'))
  h.click(h.one(node => node.type === 'button' && node.props.children === 'Use this budget'))
  url = h.url()
