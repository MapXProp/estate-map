...
const consent = load('src/lib/analyticsConsent.ts', {}, { Event })
const contact = load('src/lib/contactAnalytics.ts', { './analyticsConsent': consent })
...
const events = (win) =>
  plain((win.dataLayer || []).map((args) => Array.from(args)).filter((args) => args[0] === 'event'))
...
test('all four steps queue before tag loading, with one config and no manual extra page views', () => {
  ...
  assert.deepEqual(
    events(win).map((item) => item[1]),
    [1, 2, 3, 4].map((n) => `listing_create_step_${n}`)
  )
  assert.deepEqual(
    events(win).map((item) => item[2].step_name),
    ['property_type', 'details', 'media_price_contact', 'upload_publish']
  )
...
test('editing, auth interruption and failure remain separate from the creation success funnel', () => {
  ...
  assert.deepEqual(
    events(win).map((item) => item[1]),
    ['listing_edit_step_2', 'listing_create_auth_required', 'listing_create_error', 'listing_edit_success']
  )
...
test('only allowlisted metadata is sent; private draft data and submission keys never leave the browser', () => {
  ...
  record(
    win,
    { kind: 'success', listingId: publicId },
    {
      ...draft,
      listingTitle: 'Private title',
      listingDescription: 'Private description',
      contactName: 'Private name',
      contactEmail: 'private@example.test',
      contactPhone: '0811111111',
      lineId: 'private-line',
      latMapPosition: '13.123456',
      lngMapPosition: '100.98765',
      state: 'Private address',
      salePrice: '999999999',
      'listingPhotoUrls[]': ['https://example.test/private-photo.jpg'],
  ...
  assert.deepEqual(events(win)[0][2], {
    flow_mode: 'create',
    step_number: 4,
    step_name: 'upload_publish',
    property_type: 'land',
    listing_id: publicId,
    send_to: gaId,
    page_location: 'https://mapxprop.com/add-listing/4',
  })
  const event = analytics.buildListingFunnelEvent(
    { kind: 'step', step: 1 },
    { property_type_code: 'private@example.test' }
  )
  assert.equal(event.params.property_type, undefined)
  for (const action of [
    { kind: 'step', step: 5 },
    { kind: 'error', stage: 'private error text' },
    { kind: 'success', listingId: 'private@example.test' },
  ]) {
...
test('confirmed saves are deduplicated across retries and reloads, but separate submissions still count', () => {
  ...
  assert.equal(
    record(reload, success, { ...draft, editingPublicListingId: publicId, submissionKey: 'second-edit' }),
    true
  )
...
function submissionHarness(options = {}) {
  ...
  const react = {
    ...
    useCallback(callback) {
      if (callback.constructor.name === 'AsyncFunction') process = callback
      return callback
    },
  }
  const session = storage()
  if (options.restored)
    session.setItem('submission-result', JSON.stringify({ publicListingId: publicId, slug: 'fixture', draft: value }))
  if (options.finalizeFailure)
    session.setItem = () => {
      throw new Error('Storage full')
    }
  const page = load(
    'src/app/(app)/(other-pages)/add-listing/4/page.tsx',
    {
      react,
      ...
      '@/components/add-listing/ListingFlowProgressContext': {
        initialListingMediaProgress: progress,
        initialListingPendingMedia: pending,
      ...
      '@/lib/listingDraft': {
        LISTING_SUBMISSION_RESULT_KEY: 'submission-result',
        ListingMediaUploadError: class extends Error {},
        getListingDraft: () => value,
        saveListingStep: () => value,
        saveListingDraftToCloud: async () => {},
        clearCloudListingDraft: async () => {},
        clearListingDraft() {},
        uploadListingMedia: async () => {
          calls.upload++
          throw new Error('Private filename upload error')
        },
      ...
      '@/lib/listingPublishValidation': {
        validateListingDraftForPublish: () => options.validation || null,
...
test('failed publishing and a retry count one step view, an error and one eventual success', async () => {
  ...
  assert.deepEqual(
    events(h.win).map((item) => item[1]),
    ['listing_create_step_4', 'listing_create_error', 'listing_create_success']
  )
...
test('empty/invalid drafts, missing files, failed uploads and malformed responses cannot create conversions', async () => {
  ...
  for (const [options, failure, publishes] of cases) {
    ...
    assert.equal(
      events(h.win).some((item) => item[1].endsWith('_success')),
      false
    )
...
test('funnel events before consent or after withdrawal are dropped without dedup storage', () => {
  const win = fixtureWindow()
  delete win.__mapxpropCookieChoice
  assert.equal(record(win, { kind: 'success', listingId: publicId }), false)
  assert.equal(win.dataLayer, undefined)
  assert.equal(win.sessionStorage.getItem('mapxprop_analytics_listing_successes'), null)
  win.__mapxpropCookieChoice = { version: 1, analytics: true, updatedAt: Date.now() }
  assert.equal(record(win, { kind: 'step', step: 2 }), true)
  assert.deepEqual(
    events(win).map((item) => item[1]),
    ['listing_create_step_2']
  )
  win.__mapxpropCookieChoice.analytics = false
  assert.equal(record(win, { kind: 'step', step: 3 }), false)
  assert.equal(events(win).length, 1)
})
