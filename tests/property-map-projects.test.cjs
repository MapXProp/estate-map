...
test('map links default to listings while project deep links select project mode', () => {
  const { normalizeMapMode } = model()
  assert.equal(normalizeMapMode(''), 'listings')
  assert.equal(normalizeMapMode('', true), 'projects')
  assert.equal(normalizeMapMode('projects'), 'projects')
  assert.equal(normalizeMapMode('listings', true), 'listings')
...
test('one project view includes condos, housing estates, malls and every other registered project type', () => {
  ...
  const groups = model().groupMapProjects(
  ...
  assert.deepEqual(plain(groups.map((group) => group.category)), categories)
})
...
test('only registered membership groups pins; names and shared coordinates cannot merge unrelated buildings', () => {
  const rows = [
    listing,
    { ...listing, id: '2' },
    { ...listing, id: '3', projectPublicId: 'project-b' },
    { ...listing, id: '4', projectPublicId: '' },
  ]
  ...
  assert.deepEqual(
    plain(model().groupMapProjects([...rows].reverse())),
    plain(groups),
    'search ordering never moves the project pin'
  )
})
...
test('official project coordinate anchors units at different coordinates; unknown centers never invent a midpoint', () => {
  ...
  const group = model().groupMapProjects(
    separate.map((row) => ({ ...row, projectLatitude: 13.72, projectLongitude: 100.52 }))
  )
...
test('all project pages load independently of map filters, including units with no map coordinate', async () => {
  ...
  const rows = Array.from({ length: 125 }, (_, id) => ({
    public_listing_id: `unit-${id}`,
    project_public_id: 'project-a',
  }))
  const m = model(async (query, signal, options) => {
    calls.push({ query, options })
    return { listings: rows.slice(options.offset, options.offset + 60), total: rows.length }
  })
  assert.equal((await m.fetchMapProjectListings('project-a', new AbortController().signal)).length, 125)
  assert.deepEqual(
    calls.map((call) => call.options.offset),
    [0, 60, 120]
  )
  assert.ok(
    calls.every(
      (call) =>
        call.query === '' &&
        call.options.project === 'project-a' &&
        !call.options.view &&
        !call.options.offerTypes &&
        !call.options.propertyTypes
    )
  )
})
...
test('a stale API, broken pagination, or an empty identity cannot show unrelated listings as project units', async () => {
  ...
  await assert.rejects(
    model(async () => ({
      listings: [{ public_listing_id: 'x', project_public_id: 'project-b' }],
      total: 1,
    })).fetchMapProjectListings('project-a', signal),
    /membership/
  )
  await assert.rejects(
    model(async () => ({
      listings: [{ public_listing_id: 'x', project_public_id: 'project-a' }],
      total: 2,
    })).fetchMapProjectListings('project-a', signal),
    /pagination/
  )
  await assert.rejects(
    model(async () => ({ listings: [], total: 2 })).fetchMapProjectListings('project-a', signal),
    /Incomplete/
  )
  assert.equal(
    (await model(async () => ({ listings: [], total: 0 })).fetchMapProjectListings('project-a', signal)).length,
    0
  )
})
